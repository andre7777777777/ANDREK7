...
from tgbot.misc.states import Mailing, blockUser, UnblockUser
...
async def blocking(c: CallbackQuery):
    await c.message.answer('Отправьте айди юзера')
    await blockUser.user_id.set()

async def unblocking(c: CallbackQuery):
    await c.message.anwer('Отправьте айди юзера')
...
def register_inline(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(blocking, text='blocking', state='*')