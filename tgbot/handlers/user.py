...
async def user_start(message: Message):
    try:
        ...
        pass
    users = db.select_all_users_in_block()
    if int(message.from_user.id) in users:
        pass
    else:
...
async def blocking_user(message, state: FSMContext):
    bot = Bot(token=config.tg_bot.token, parse_mode='HTML')
    if message.text.isdigit():
        user_id = int(message.text)
        users = db.select_all_users()
        if user_id in users:
            db.add_user_to_block(user_id)
            await bot.send_message(message.from_user.id, 'Успешно занес человека в черный список!')
            await state.finish()
        else:
            await bot.send_message(message.from_user.id, 'Данного человека нету в БД!')
            await state.finish()
    else:
        await bot.send_message(message.from_user.id, 'Нельзя использовать буквы!')
        await state.finish()

async def unblocking_user(message, state: FSMContext):
    bot = Bot(token=config.tg_bot.token, parse_mode='HTML')
    if message.text.isdigit():
        user_id = int(message.text)
        users = db.select_all_users_in_block()
        if user_id in users:
            db.delete_user_in_block(user_id)
            await bot.send_message(message.from_user.id, 'Успешно вынес человека из черного списка!')
            await state.finish()
        else:
            await bot.send_message(message.from_user.id, 'Данного человека нету в БД!')
            await state.finish()
